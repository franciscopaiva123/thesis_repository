...
print("Normalized data 5", len(dataset_normalized_5))
...
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=3, padding='same', activation='relu', input_shape=(total_time_steps, n_features)))
...
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=3))
model.add(Conv1D(filters=254, kernel_size=3,padding='same', activation='relu'))
model.add(Dropout(0.2))
model.add(BatchNormalization())
model.add(MaxPooling1D(pool_size=3))
model.add(Flatten())
...
model.add(Dropout(0.2))
model.add(Dense(n_classes, activation='softmax'))  # Updated for multi-class classification

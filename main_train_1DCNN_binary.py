...
def normalize(df):
    ...
    exclude_cols = ['mmsi', 'timestamp','is_fishing']
...
def segment_vessel_data(df, segment_length=45, max_time_hours=1, min_valid_signals=6, label=''):
    all_segments = []
...
model.add(Dropout(0.2))
model.add(Dense(1, activation='sigmoid'))  # Updated for multi-class classification
